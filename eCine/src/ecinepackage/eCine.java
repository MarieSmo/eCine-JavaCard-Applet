...
public class eCine extends Applet {
	...
	public void process(APDU apdu) throws ISOException {
		...
		case INS_VERIFY_PIN:
			...
			break;
		case INS_GET_LOGS:
		    Util.arrayCopyNonAtomic(logger.toByteArray(), (short)0, buffer, (short)0, (short) (Logger.MAX_LOG*Logger.MESSAGE_SIZE));
			apdu.setOutgoingAndSend((short) 0, (short) (Logger.MAX_LOG*Logger.MESSAGE_SIZE));
			break;
		default:
	...
	// -------------------- HANDLE PIN/PUK --------------------------------
	// -------------------- HANDLE TICKETS --------------------------------
	private boolean isBetween(short fdStart, byte fdDuration, short sdStart, byte sdDuration) {
		...
		if (s1.getYear() == s2.getYear())
			if (s1.getMonth() == s2.getMonth())
				if (s1.getDay() == s2.getDay())
						return isBetween(s1.getTime(), s1.getDuration(),s2.getTime(), s2.getDuration());
		return false;
	}
	...
	private void addScreening(Purchase p) {
		short insertIndex = -1;
		// Check overlap 
		if(immediateScreening != null && checkOverlaping(immediateScreening, p.getScreening())) {
			ISOException.throwIt(SW2_DATE_CONFLICT);				
		}
		for (short i = 0; i < 5; ++i) {
			if (screenings[i] == null && insertIndex == -1) {
				insertIndex = i;
			} else if(screenings[i] != null  && checkOverlaping(screenings[i], p.getScreening())) {
				ISOException.throwIt(SW2_DATE_CONFLICT);				
			}
		}
		...
		if (purchase.getScreening().getYear() == purchase.getTdYear())
			if (purchase.getScreening().getMonth() == purchase.getTdMonth())
				if (purchase.getScreening().getDay() == purchase.getTdDay()) {
						...
						return (purchase.getTdTime() - purchase.getScreening().getTime() <= 30 
								|| (purchase.getTdTime() - purchase.getScreening().getTime() <= -60));
				}
		return false;
	}
	...
	private void buyTicket(APDU apdu) {
		...
		if(rewards == 50) {
			rewards = 0;
			logger.logTicketPurchase(newTicket.getIDMovie(), newTicket.getPrice(), Logger.BUY_TICKET_REWARD);
		} else {
			rewards+=5;
			balance = (byte) (balance - newTicket.getPrice());
			logger.logTicketPurchase(newTicket.getIDMovie(),newTicket.getPrice(), Logger.BUY_TICKET_BALANCE);
		}
	}
	// -------------------- HANDLE TICKETS --------------------------------
}
